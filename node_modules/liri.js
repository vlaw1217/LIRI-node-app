...
//console.log("xxxxxxxx: "+liriInput)
...
switch (liriInput) {
    ...
    default:
        console.log
            (`-----------------------------------------
        Please type one as below then search name
        1. song-this
        2. movie-this
        3. concert-this
        4. do-what-it-says
...
Release Date: ${moment(output.album.release_date).format('MM/DD/YYYY')}`
            ...
            fs.appendFile('log.txt', dataOutput, (err) => {
                if (err) throw err;
...
function movieThis(doWhatItSayValue) {
    ...
    if (process.argv[3] === undefined) {
        if (liriInput === "movie-this") {
            logType = "Type of Action: Movie-This\n"
            movieName = "Mr.+Nobody"
        } else {
            logType = "Type of Action: Do-What-It-Say\n"
            movieName = doWhatItSayValue
        }
    } else {
        logType = "Type of Action: Movie-This\n"
    }
...
Plot: ${outputOmdb.Plot}`
            ...
            dataOutputOmdb = logType + dataOutputOmdb + "\n\n";

            fs.appendFile('log.txt', dataOutputOmdb, (err) => {
                if (err) throw err;
                console.log('The "data to append" was appended to file!');
            });
...
Date: ${moment(bandsInTown.datetime).format('MM/DD/YYYY')}`
            ...
            console.log("===================================================================")
            bandsInTownOutput = bandsInTownOutput + "\n\n";
            fs.appendFile('log.txt', bandsInTownOutput, (err) => {
                if (err) throw err;
                console.log('The "data to append" was appended to file!');
            });
        })
...
function doWhatItSays() {
    ...
    //console.log("insidexxxxxxxxxxxx")
    //let randomTxt = "random.txt"
    if (process.argv[2] == "do-what-it-says") {
        fs.readFile("random.txt", "utf8", function (err, data) {
            dataShown = data.split(",");
            movieName = dataShown[1];
            if (dataShown[0] === "movie-this") {
                movieThis(movieName);
